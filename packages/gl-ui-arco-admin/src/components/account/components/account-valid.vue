...
import {userApi, authUtil,stringUtil} from "@geelato/gl-ui";
...
const validTypeData = () => {
  ...
  if (userStore.mobilePhone) {
    options.push({
      value: '1',
      label: `${t('account.manage.valid.validType.mobile.prefix')} ${userStore.mobilePrefix} ${stringUtil.abbreviateValue(userStore.mobilePhone, '1')} ${t('account.manage.valid.validType.suffix')}`
    });
  }
  if (userStore.email) {
    options.push({
      value: '2',
      label: `${t('account.manage.valid.validType.email.prefix')} ${stringUtil.abbreviateValue(userStore.email, '2')} ${t('account.manage.valid.validType.suffix')}`
    });
  }
...
const nextModelClick = async (ev?: MouseEvent) => {
  ...
  if (!res) {
    ...
    try {
      await userApi.validateUser(formData.value);
      authUtil.setValidUser(formData.value.userId);
      emits('update:modelValue', false);
      emits('validEvent');
    } catch (err) {
...
<template>
  <a-modal draggable  v-model:visible="visible" :title="$t('account.manage.index.title')"
    ...
    <a-form ref="validFormRef" :model="formData" :wrapper-col-props="{ span: 24 }" class="form" size="large">
      ...
      <a-form-item
          ...
          field="authCode">
        <a-input-password v-model="formData.authCode" :placeholder="$t('account.manage.valid.password.placeholder')" allow-clear/>
      </a-form-item>
    </a-form>
    <template #footer>
      <a-space>
