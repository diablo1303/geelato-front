...
import type {AccountUserInfo} from "@geelato/gl-ui";
import {useUserStore} from "../../../store";
import VueCropping from "../../vue-cropper/index.vue";
import {uploadFile} from "../../vue-cropper/type";
...
const updateUserInfoClick = async (ev?: MouseEvent) => {
  ...
  try {
    await userApi.updateUserInfo(userStore.id as string, formData.value);
    userStore.info();
    Message.success(t('account.user.form.msg.success'));
  } catch (err) {
    console.log(err);
  } finally {
