...
import {IMAGE_MIME, uploadFile} from "./type";
import type {CropperControllerData, CropperOptions} from "./type";
...
const okModalClick = async (ev?: MouseEvent) => {
  cropperRef.value.getCropData(async (data: string) => {
    ...
    try {
      ...
      await userApi.uploadAvatar(userStore.id as string, formData);
      emits('update:modelValue', false);
      emits('completeEvent');
    } catch (err) {
      console.log(err);
...
<template>
  <a-modal draggable  v-model:visible="visible"
           :title="$t('account.user.cropper.modal.title')"
           title-align="start"
           width="600px"
           @cancel="cancelModalClick">
