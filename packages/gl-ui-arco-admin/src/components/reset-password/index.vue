...
import type {AuthCodeForm, ResetPasswordForm} from "@geelato/gl-ui";
...
const validNextClick = async (ev?: MouseEvent) => {
  ...
  if (!res) {
    ...
    try {
      const {data} = await userApi.forgetPasswordValid(formData.value);
      console.log(data);
      // @ts-ignore
...
const authCodeClick = async (ev?: MouseEvent) => {
  if (formData.value.userId) {
    ...
    try {
      await userApi.generateAuthCode(otherParams as AuthCodeForm);
      authCodeInterval();
    } catch (err) {
...
const updatePwdClick = async (ev?: MouseEvent) => {
  ...
  if (!res) {
    ...
    try {
      await userApi.forgetPasswordEdit(formData.value);
      errorMessage.value = '';
      step.value = 3;
      if (redirect) {
    ...
    <keep-alive class="reset-form-keep-alive">
      <a-form ref="validFormRef" :model="formData" :wrapper-col-props="{ span: 24 }" class="form" size="large">
        ...
        <a-form-item
          ...
          <a-input v-model="formData.validBox" :disabled="step===2" :placeholder="$t('reset.password.validBox.mobile.placeholder')" allow-clear/>
        </a-form-item>
        <a-form-item
            ...
            field="validBox">
          <a-input v-model="formData.validBox" :disabled="step===2" :placeholder="$t('reset.password.validBox.email.placeholder')" allow-clear/>
        </a-form-item>
        <!--     重置密码     -->
        <a-form-item
            v-show="step===2"
        ...
        <a-form-item
            ...
            field="password">
          <a-input-password v-model="formData.password" :placeholder="$t('reset.password.password.placeholder')" allow-clear/>
        </a-form-item>
        <a-form-item
            v-show="step===2"
            :hide-asterisk="true"
