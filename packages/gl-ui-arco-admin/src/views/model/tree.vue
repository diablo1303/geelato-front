...
import type {TreeNodeProps} from "@arco-design/web-vue/es/tree/interface";
import {IconFolder, IconLink} from '@arco-design/web-vue/es/icon';
import {emitter, modelApi} from "@geelato/gl-ui";
import type {PageQueryRequest, QueryConnectForm, QueryTableForm} from "@geelato/gl-ui";
...
const fetchConnects = async (params: PageQueryRequest = {current: 1, pageSize: 10000}): Promise<TreeNode[]> => {
  ...
  try {
    const {data} = await modelApi.queryConnects({
      ...params,
