...
import {isUtil, stringUtil, applicationApi, modelApi, securityApi, dictApi, encodingApi} from "@geelato/gl-ui";
import type {
  PageQueryRequest,
  ColumnSelectType,
...
import DictionaryLayout from "../../security/dictionary/layout.vue";
 ...
 * @param failBack
 */
const saveData = async (params: QueryTableColumnForm, successBack?: any, failBack?: any) => {
...
const saveOrUpdate = (successBack?: any, failBack?: any) => {
  saveData(formData.value, (data: QueryTableColumnForm) => {
    // 设计当前页面的操作
    if (successBack && typeof successBack === 'function') successBack(data);
  }, () => {
    ...
    <a-row :gutter="wrapperCol">
      ...
      <a-col v-if="['ORG','USER'].includes(formData.selectType)" :span="(labelCol+wrapperCol)/formCol">
        <a-form-item :label="$t('model.column.index.form.autoAdd')"
                     ...
                     field="autoAdd">
          <a-checkbox-group v-model="formData.autoAdd as string[]"
                            :max="1"
