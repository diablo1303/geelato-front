...
import type {PageQueryFilter, PageQueryRequest, FormParams, QueryTableForm, QueryViewForm} from '@geelato/gl-ui';
// 页面所需 对象、方法
 ...
 * @param data
 */
const viewTable = (data: QueryViewForm) => {
  formParams.value = Object.assign(formParams.value, {
 ...
 * @param data
 */
const editTable = (data: QueryViewForm) => {
  formParams.value = Object.assign(formParams.value, {
  ...
  <a-table
    ...
    <template #columns>
      ...
      <a-table-column v-show="formState==='edit'" :title="$t('model.view.index.form.operations')" :width="360" align="center"
        ...
        <template #cell="{ record }">
          <a-button v-if="record.viewType==='default' || formState==='view'" size="small" type="text" @click="viewTable(record)">
