...
import {applicationApi, modelApi} from "@geelato/gl-ui";
import type {QueryAppForm, QueryConnectForm} from "@geelato/gl-ui";
...
/* 表单 */
const generateFormData = (): QueryConnectForm => {
  return {
    id: props.modelValue || '',
    dbConnectName: '',
    dbName: '',
 ...
 * @param failBack
 */
const saveData = async (params: QueryConnectForm, successBack?: any, failBack?: any) => {
  const res = await validateForm.value?.validate();
  if (!res) {
    try {
      params.apps = appSelectData.value && appSelectData.value.toString();
      const {data} = await modelApi.createOrUpdateConnect(params);
...
const getData = async (id: string, successBack?: any, failBack?: any) => {
  try {
    const {data} = await modelApi.getConnect(id);
    if (successBack && typeof successBack === 'function') successBack(data);
  } catch (err) {
    if (failBack && typeof failBack === 'function') failBack(err);
  }
 ...
 * @param failBack
 */
const connectionTest = async (params: QueryConnectForm, successBack?: any, failBack?: any) => {
  const res = await validateForm.value?.validate();
  if (!res) {
    try {
      const {data} = await modelApi.jdbcConnect(params);
      if (successBack && typeof successBack === 'function') successBack(data);
    } catch (err) {
      if (failBack && typeof failBack === 'function') failBack(err);
    } finally {
...
const saveOrUpdate = (successBack?: any, failBack?: any) => {
  saveData(formData.value, (data: QueryConnectForm) => {
    // 设计当前页面的操作
    if (successBack && typeof successBack === 'function') successBack(data);
  }, () => {
    if (failBack && typeof failBack === 'function') failBack();
...
const loadPage = () => {
  ...
  if (['edit', 'view'].includes(props.formState) && props.modelValue) {
    getData(props.modelValue, (data: QueryConnectForm) => {
