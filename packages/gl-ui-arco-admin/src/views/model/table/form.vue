...
// 引入组件
import ModelTableModel from './model.vue';
...
const tableFormRef = shallowRef(ModelTableModel);
...
<template>
  <a-modal draggable  v-if="isModal"
           ...
           :title="title || $t(`model.table.index.model.title.${formState}`)"
           :width="width || ''" title-align="start"
           @cancel="handleModelCancel($event)" @before-ok="handleModelOk">
    <ModelTableModel ref="tableFormRef"
