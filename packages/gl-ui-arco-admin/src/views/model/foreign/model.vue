...
import type {PageQueryRequest, QueryAppForm, QueryTableColumnForm, QueryTableForeignForm, QueryTableForm} from "@geelato/gl-ui";
 ...
 * @param failBack
 */
const saveData = async (params: QueryTableForeignForm, successBack?: any, failBack?: any) => {
...
const fetchTableColumns = async (params?: Record<string, any>): Promise<QueryTableColumnForm[]> => {
  ...
  try {
    const {data} = await modelApi.queryTableColumns({...params, enableStatus: 1} as unknown as PageQueryRequest);
    columnOptions = data;
  } catch (err) {
    columnOptions = [];
  }
...
const saveOrUpdate = (successBack?: any, failBack?: any) => {
  saveData(formData.value, (data: QueryTableForeignForm) => {
    // 设计当前页面的操作
    if (successBack && typeof successBack === 'function') successBack(data);
  }, () => {
