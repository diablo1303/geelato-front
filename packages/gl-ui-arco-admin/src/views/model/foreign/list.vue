...
import type {PageQueryFilter, PageQueryRequest, FormParams, QueryTableForeignForm} from '@geelato/gl-ui';
// 页面所需 对象、方法
...
const fetchData = async (params: PageQueryRequest) => {
  setLoading(true);
  try {
    const {data} = await modelApi.pageQueryTableForeigns(params);
 ...
 * @param data
 */
const viewTable = (data: QueryTableForeignForm) => {
  formParams.value = Object.assign(formParams.value, {
 ...
 * @param data
 */
const editTable = (data: QueryTableForeignForm) => {
  formParams.value = Object.assign(formParams.value, {
