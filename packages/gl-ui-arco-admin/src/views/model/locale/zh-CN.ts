export default {
  ...
  'model.table.index.form.packBusData.tip.1': '用于部署时，只插入本次打包的数据，不修改不删除目标表数据。',
  'model.table.index.form.packBusData.2': '全量打包',
  'model.table.index.form.packBusData.tip.2': '用于部署时，清空目标表之后插入本次打包的数据；一般为需要在生产环境并行维护的数据，如系统的参数。',
  'model.table.index.form.packBusData.extra': '打包业务数据，用于插入时。增量插入（只插入本次打包的数据，不修改不删除目标表数据。）；全量更新（清空目标表之后插入本次打包的数据；一般为需要在生产环境并行维护的数据，如系统的参数。）',
