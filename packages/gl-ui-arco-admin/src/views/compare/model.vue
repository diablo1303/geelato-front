...
const props = defineProps({
  modelValue: {type: Object, default: () => ({} as QueryAppVersionForm)},
...
<template>
  <a-form :label-col-props="{ span: '2px' }" :model="renderData" class="form" style="padding-left: 16px;">
    ...
    <a-form-item field="packageSource" label="版本来源：">
      <span>{{ utils.getOptionLabel(renderData.packageSource, packageSourceOptions) }}</span>
    </a-form-item>
    <a-form-item field="status" label="版本状态：">
      <span>{{ utils.getOptionLabel(renderData.status, packageStatusOptions) }}</span>
    </a-form-item>
