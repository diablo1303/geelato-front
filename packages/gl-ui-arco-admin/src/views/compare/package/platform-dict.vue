...
import {directions, queryCompareType, sortRenderData, getDiffData, objValueToArray,} from "../type";
...
// 树默认根节点
const rootNode = {title: "数据字典管理", key: 'root', level: 0, data: {}, children: [], retain: true};
// 对比组件ID
...
const queryTreeSecondItems = (direction: string, record: TreeNodeModel, data: TreeLevelData, compare: TreeLevelData) => {
  ...
  if (data && data.second && data.second.length > 0) {
    ...
    for (const item of data.second.filter(item => (item.dict_id === record.key && isUtil.isBlank(item.pid)) || (item.dict_id === record?.data?.dict_id && item.pid === record.key))) {
...
const queryRenderData = (list: AppMeta[], data: TreeLevelData) => {
  ...
  sortRenderData(data.second, 'seq_no|asc,del_status|asc');
  // 层级和表对应
  data.level = {1: "platform_dict", 2: "platform_dict_item"};
