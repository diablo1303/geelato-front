...
import type {PageQueryFilter, PageQueryRequest, QueryAppForm} from '@geelato/gl-ui';
import {applicationApi} from '@geelato/gl-ui'
...
const fetchData = async (params: PageQueryRequest) => {
  setLoading(true);
  try {
    const {data} = await applicationApi.pageQueryApps(params);
...
const deleteData = async (id: string, successBack?: any, failBack?: any) => {
  try {
    await applicationApi.deleteApp(id);
...
/* 列表，按钮、操作列 */
const enterTable = (data: QueryAppForm, type: string) => {
  if (data && router) {
...
const viewTable = (data: QueryAppForm) => {
...
const editTable = (data: QueryAppForm) => {
  formParams.value = Object.assign(formParams.value, {
    id: data.id, visible: true, formState: 'edit'
  });
}
const deleteTable = (data: QueryAppForm) => {
 ...
 * @param type
 */
const saveSuccess = (data: QueryAppForm, type: string) => {
  if (type === 'add') {
    reset();
    emits('add', data);
  } else if (type === 'edit') {
