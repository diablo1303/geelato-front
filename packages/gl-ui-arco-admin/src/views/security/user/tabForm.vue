...
const formData = ref<QueryUserForm>({} as unknown as QueryUserForm);
const tableFormRef = shallowRef(UserModel);
const tabsKey = ref<number>(1);// 定位tabs页面
const loadUserOrg = ref<string>(utils.generateRandom());
...
const updateForm = () => {
  ...
  if (tableFormRef.value && typeof tableFormRef.value?.saveOrUpdate === 'function') {
    // @ts-ignore
    tableFormRef.value?.saveOrUpdate((data: QueryUserForm) => {
      setLoading(false);
      Message.success("更新成功！");
      if (formData.value.orgName !== data.orgName) {
        loadUserOrg.value = utils.generateRandom();
        setTimeout(() => {
          tabsKey.value = 2;
        }, 3 * 1000);
      }
