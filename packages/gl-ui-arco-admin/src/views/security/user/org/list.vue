...
import type {PageQueryFilter, PageQueryRequest, QueryOrgUserForm, QueryOrgForm} from '@geelato/gl-ui';
// 页面所需 对象、方法
import useLoading from '../../../../hooks/loading';
import {type Pagination, PageSizeOptions} from '../../../../types/global';
import {categoryOptions, statusOptions, typeOptions} from "../../org/searchTable";
...
const fetchData = async (params: PageQueryRequest) => {
  setLoading(true);
  try {
    const {data} = await securityApi.pageQueryOrgUserOf(params);
...
const deleteData = async (id: string, successBack?: any, failBack?: any) => {
  try {
    await securityApi.deleteOrgUser(id);
 ...
 * @param data
 */
const deleteTable = (data: QueryOrgUserForm) => {
...
const confirmModal = async (data: QueryOrgForm[]) => {
  if (data && data.length > 0) {
    ...
    try {
      await securityApi.insertOrgUser({
        userId: props.parameter.userId,
        orgId: orgIds.join(',')
...
<template>
        ...
        <a-row :gutter="wrapperCol">
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('security.org.index.form.name')" field="orgName">
              <a-input v-model="filterData.orgName" allow-clear @clear="condition" @press-enter="condition"/>
            </a-form-item>
          </a-col>
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('security.org.index.form.code')" field="orgCode">
              <a-input v-model="filterData.orgCode" allow-clear @clear="condition" @press-enter="condition"/>
            </a-form-item>
          </a-col>
          <a-col :span="(labelCol+wrapperCol)/filterCol">
  ...
  <GlOrgSelect v-model:model-value="selectData"
