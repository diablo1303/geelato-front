...
import type {PageQueryFilter, PageQueryRequest, FormParams, QueryUserForm} from '@geelato/gl-ui';
// 页面所需 对象、方法
...
const scroll = ref({x: 2900, y: props.height});
// 列表 - 排序
 ...
 * @param data
 */
const viewTable = (data: QueryUserForm) => {
 ...
 * @param data
 */
const editTable = (data: QueryUserForm) => {
  tabsformParams.value = Object.assign(tabsformParams.value, {
 ...
 * @param data
 */
const deleteTable = (data: QueryUserForm) => {
 ...
 * @param record
 */
const resetPwdTable = async (record: QueryUserForm) => {
  try {
    const {data} = await securityApi.resetPassword(record.id);
    // @ts-ignore
 ...
 * @param type
 */
const saveSuccess = (data: QueryUserForm, type: string) => {
  if (type === 'add') {
    reset();
    emits('add', data);
  } else if (type === 'edit') {
...
<template>
        ...
        <a-row :gutter="wrapperCol">
          ...
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('security.user.index.form.loginName')" field="loginName">
              <a-input v-model="filterData.loginName" allow-clear @clear="condition" @press-enter="condition"/>
            </a-form-item>
          </a-col>
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('security.user.index.form.orgName')" field="code">
              ...
              <a-input v-else v-model="filterData.orgName" allow-clear @clear="condition" @press-enter="condition"/>
            </a-form-item>
          </a-col>
          <a-col :span="(labelCol+wrapperCol)/filterCol">
  ...
  <a-table
    ...
    <template #columns>
      ...
      <a-table-column :title="$t('security.user.index.form.operations')" :width="300" align="center" data-index="operations" fixed="right">
        <template #cell="{ record }">
