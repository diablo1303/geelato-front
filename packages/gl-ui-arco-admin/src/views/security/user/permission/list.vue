...
import type {PageQueryFilter, PageQueryRequest, QueryRoleForm, QueryRoleUserForm} from '@geelato/gl-ui';
// 页面所需 对象、方法
...
import RoleTabsForm from "../../role/tabsForm.vue";
...
const fetchData = async (params: PageQueryRequest) => {
  setLoading(true);
  try {
    const {data} = await securityApi.pageQueryUserOf(params);
    // @ts-ignore
...
const switchRU = async (params?: Record<string, any>, successBack?: any, failBack?: any) => {
  try {
    await securityApi.switchRoleUser({...params} as QueryRoleUserForm);
...
const insertRU = async (params?: Record<string, any>, successBack?: any, failBack?: any) => {
  try {
    await securityApi.insertRoleUser({...params} as QueryRoleUserForm);
    if (successBack && typeof successBack === 'function') successBack();
  } catch (err) {
    if (failBack && typeof failBack === 'function') failBack();
  }
...
<template>
  <a-modal draggable  v-model:visible="listParams.visible"
           :footer="false"
          ...
          <template #title>
            <a-popover :title="item.name" position="tr" style="max-width: 300px">
              ...
              <span>
                ...
                <CopyToClipboard v-if="item.code" :model-value="item.code" title="点击复制编码"/>
              </span>
                <br/>
                <span>
