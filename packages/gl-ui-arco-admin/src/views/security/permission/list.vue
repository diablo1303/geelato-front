...
import {securityApi, applicationApi} from '@geelato/gl-ui'
import type {QueryAppForm, PageQueryFilter, PageQueryRequest, QueryPermissionForm} from '@geelato/gl-ui';
// 页面所需 对象、方法
...
const fetchData = async (params: PageQueryRequest) => {
  setLoading(true);
  try {
    const {data} = await securityApi.pageQueryPermission(params);
 ...
 * @param data
 */
const viewTable = (data: QueryPermissionForm) => {
  formParams.value = Object.assign(formParams.value, {
 ...
 * @param data
 */
const editTable = (data: QueryPermissionForm) => {
  formParams.value = Object.assign(formParams.value, {
 ...
 * @param data
 */
const deleteTable = (data: QueryPermissionForm) => {
 ...
 * @param type
 */
const saveSuccess = (data: QueryPermissionForm, type: string) => {
  if (type === 'add') {
    reset();
    emits('add', data);
  } else if (type === 'edit') {
        ...
        <a-row :gutter="wrapperCol">
          ...
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('security.permission.index.form.object')" field="object">
              <a-input v-model="filterData.object" allow-clear @clear="condition" @press-enter="condition"/>
            </a-form-item>
          </a-col>
          <a-col :span="(labelCol+wrapperCol)/filterCol">
