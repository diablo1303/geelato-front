...
import {utils, securityApi, applicationApi} from "@geelato/gl-ui";
      ...
      <a-col :span="(labelCol+wrapperCol)">
        <a-form-item :label="$t('security.permission.index.form.rule')"
          ...
          <template v-if="formState!=='view'" #extra>
            <a-descriptions :column="formCol" :bordered="true" layout="horizontal" size="small">
              <a-descriptions-item v-for="(item,index) of ruleOptions" :key="index" :label="item.label">
                <CopyToClipboard v-if="item.value" :model-value="item.value as string"/>
                {{ item.value }}
              </a-descriptions-item>
            </a-descriptions>
          </template>
