...
import type {TableData, SelectOptionData, CascaderOption, FormInstance} from "@arco-design/web-vue";
import {utils, stringUtil, encodingApi, applicationApi} from '@geelato/gl-ui'
import type {QueryAppForm, EncodingItem, QueryEncodingForm} from '@geelato/gl-ui';
import {dateTypeOptions, enableStatusOptions, separatorsOptions, serialTypeOptions, variableTypeOptions} from "./searchTable";
...
/* 表单 */
const generateFormData = (): QueryEncodingForm => {
  return {
    id: props.modelValue || '',
    title: '',
    separators: '',
 ...
 * @param failBack
 */
const createOrUpdateData = async (params: QueryEncodingForm, successBack?: any, failBack?: any) => {
  const res = await validateForm.value?.validate();
  if (!res && validateTemplate()) {
    try {
      params.template = JSON.stringify(templateData.value);
      const {data} = await encodingApi.createOrUpdateEncoding(params);
...
const getData = async (id: string, successBack?: any, failBack?: any) => {
  try {
    const {data} = await encodingApi.getEncoding(id);
    if (successBack && typeof successBack === 'function') successBack(data);
  } catch (err) {
    if (failBack && typeof failBack === 'function') failBack(err);
  }
...
const addTemplate = (type: string) => {
  switch (type) {
    case 'constant':
      templateData.value.push({id: utils.generateRandom(), itemType: type, seqNo: templateData.value.length + 1, constantValue: ''} as unknown as EncodingItem);
      break;
    case 'variable':
      templateData.value.push({id: utils.generateRandom(), itemType: type, seqNo: templateData.value.length + 1, constantValue: ''} as unknown as EncodingItem);
      break;
        ...
        case 'serial':
          ...
          if (serialDigitOptions.value.includes(serialDigit) && item.serialType) {
            if (item.serialType === 'order') {
              ...
              example.push(utils.generateRandom(serialDigit));
            }
          }
          break;
        case 'date':
          if (item.dateType)
            example.push(stringUtil.formatTime(new Date(), item.dateType));
          break;
        default:
          break;
      }
...
const saveOrUpdate = (successBack?: any, failBack?: any) => {
  createOrUpdateData(formData.value, (data: QueryEncodingForm) => {
    // 设计当前页面的操作
    if (successBack && typeof successBack === 'function') successBack(data);
  }, () => {
...
const loadPage = () => {
  ...
  if (['edit', 'view'].includes(props.formState) && props.modelValue) {
    getData(props.modelValue, (data: QueryEncodingForm) => {
      // 表格数据处理
      formData.value = data;
