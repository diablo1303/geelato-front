...
import type {PageQueryFilter, PageQueryRequest, QueryDictForm} from '@geelato/gl-ui';
...
const fetchData = async (params: PageQueryRequest) => {
  setLoading(true);
  try {
    const {data} = await dictApi.pageQueryDict(params);
 ...
 * @param data
 */
const viewTable = (data: QueryDictForm) => {
  formParams.value = Object.assign(formParams.value, {
 ...
 * @param data
 */
const editTable = (data: QueryDictForm) => {
  formParams.value = Object.assign(formParams.value, {
 ...
 * @param data
 */
const configTable = (data: QueryDictForm) => {
  layoutParams.value = Object.assign(layoutParams.value, {
 ...
 * @param data
 */
const deleteTable = (data: QueryDictForm) => {
 ...
 * @param _action
 */
const saveSuccess = (data: QueryDictForm, _action: string) => {
  if (_action === 'add') {
    reset();
    emits('add', data);
        ...
        <a-row :gutter="wrapperCol">
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('security.dict.index.form.dictName')" field="dictName">
              <a-input v-model="filterData.dictName" allow-clear @clear="condition" @press-enter="condition"/>
            </a-form-item>
          </a-col>
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('security.dict.index.form.dictCode')" field="dictCode">
              <a-input v-model="filterData.dictCode" allow-clear @clear="condition" @press-enter="condition"/>
            </a-form-item>
          </a-col>
          <a-col :span="(labelCol+wrapperCol)/filterCol">
  ...
  <a-table
    ...
    <template #columns>
      ...
      <a-table-column v-show="formState==='edit'" :title="$t('security.dict.index.form.operations')" :width="260" align="center" data-index="operations"
                      fixed="right">
        <template #cell="{ record }">
          <a-button size="small" type="text" @click="dictApi.exportDictAndItems(record.id)">
