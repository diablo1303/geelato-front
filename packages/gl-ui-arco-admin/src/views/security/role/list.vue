...
import type {PageQueryFilter, PageQueryRequest, QueryRoleForm} from '@geelato/gl-ui';
...
const fetchData = async (params: PageQueryRequest) => {
  ...
  try {
    // @ts-ignore
    const {data} = params.appId ? await securityApi.pageQueryOfRole(params) : await securityApi.pageQueryRole(params);
...
const relieveData = async (id: string, successBack?: any, failBack?: any) => {
  try {
    await securityApi.deleteRoleOfApp(props.parameter.appId, id);
    if (successBack && typeof successBack === 'function') successBack(id);
  } catch (err) {
    if (failBack && typeof failBack === 'function') failBack(err);
  }
 ...
 * @param data
 */
const viewTable = (data: QueryRoleForm) => {
 ...
 * @param data
 */
const editTable = (data: QueryRoleForm) => {
  tabsformParams.value = Object.assign(tabsformParams.value, {
 ...
 * @param data
 */
const deleteTable = (data: QueryRoleForm, type?: string) => {
  if (type === 'of') {
 ...
 * @param type
 */
const saveSuccess = (data: QueryRoleForm, type: string) => {
  if (type === 'add') {
    reset();
    emits('add', data);
  } else if (type === 'edit') {
        ...
        <a-row :gutter="wrapperCol">
          ...
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('security.role.index.form.weight')" field="weight">
              <a-input v-model="filterData.weight" allow-clear @clear="condition" @press-enter="condition"/>
            </a-form-item>
          </a-col>
