...
import {typeOptions} from '../../permission/searchTable';
...
const fetchData = async (params: PageQueryRequest) => {
  setLoading(true);
  try {
    const {data} = await securityApi.pageQueryRolePermissionOf(params);
...
const deleteData = async (id: string, successBack?: any, failBack?: any) => {
  try {
    await securityApi.deleteRolePermission(id);
        ...
        <a-row :gutter="wrapperCol">
          ...
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('security.permission.index.form.code')" field="permissionCode">
              <a-input v-model="filterData.permissionCode" allow-clear @clear="condition" @press-enter="condition"/>
            </a-form-item>
          </a-col>
          <a-col :span="(labelCol+wrapperCol)/filterCol">
          ...
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('security.permission.index.form.object')" field="permissionObject">
              <a-input v-model="filterData.permissionObject" allow-clear @clear="condition" @press-enter="condition"/>
            </a-form-item>
          </a-col>
        </a-row>
      </a-form>
