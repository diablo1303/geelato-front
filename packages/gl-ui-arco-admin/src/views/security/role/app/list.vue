...
import {applicationApi, securityApi} from '@geelato/gl-ui'
import type {PageQueryFilter, PageQueryRequest, QueryAppForm, QueryRoleAppForm} from '@geelato/gl-ui';
// 页面所需 对象、方法
import useLoading from '../../../../hooks/loading';
import {type Pagination, PageSizeOptions} from '../../../../types/global';
import favicon from "../../../../assets/favicon.ico";
import {statusOptions, watermarkOptions} from "../../../application/searchTable";
...
const deleteData = async (id: string, successBack?: any, failBack?: any) => {
  try {
    await securityApi.deleteRoleApp(id);
...
const closeTrigger = async (ev?: MouseEvent) => {
  if (selectData.value.length > 0) {
    try {
      await securityApi.insertRoleApp({
        roleId: props.parameter.roleId || '',
        ...
        <a-row :gutter="wrapperCol">
          ...
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('application.app.list.code')" field="appCode">
              <a-input v-model="filterData.appCode" allow-clear @clear="condition" @press-enter="condition"/>
            </a-form-item>
          </a-col>
          <a-col :span="(labelCol+wrapperCol)/filterCol">
