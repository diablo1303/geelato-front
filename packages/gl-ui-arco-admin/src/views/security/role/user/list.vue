...
import type {PageQueryFilter, PageQueryRequest, QueryRoleUserForm, QueryUserForm} from '@geelato/gl-ui';
// 页面所需 对象、方法
import useLoading from '../../../../hooks/loading';
import {type Pagination, PageSizeOptions} from '../../../../types/global';
import {enableStatusOptions, sexOptions, typeOptions} from "../../user/searchTable";
...
const fetchData = async (params: PageQueryRequest) => {
  setLoading(true);
  try {
    const {data} = await securityApi.pageQueryRoleUserOf(params);
...
const deleteData = async (id: string, successBack?: any, failBack?: any) => {
  try {
    await securityApi.deleteRoleUser(id);
...
const confirmModal = async (data: QueryUserForm[]) => {
  if (data && data.length > 0) {
    ...
    try {
      await securityApi.insertRoleUser({
        roleId: props.parameter.roleId,
        userId: userIds.join(',')
...
<template>
        ...
        <a-row :gutter="wrapperCol">
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('security.user.index.form.name')" field="userName">
              <a-input v-model="filterData.userName" allow-clear @clear="condition" @press-enter="condition"/>
            </a-form-item>
          </a-col>
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('security.user.index.form.enName')" field="userEnName">
              <a-input v-model="filterData.userEnName" allow-clear @clear="condition" @press-enter="condition"/>
            </a-form-item>
          </a-col>
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('security.user.index.form.loginName')" field="userLoginName">
              <a-input v-model="filterData.userLoginName" allow-clear @clear="condition" @press-enter="condition"/>
            </a-form-item>
          </a-col>
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('security.user.index.form.orgName')" field="userOrgName">
              <a-input v-model="filterData.userOrgName" allow-clear @clear="condition" @press-enter="condition"/>
            </a-form-item>
          </a-col>
          <a-col :span="(labelCol+wrapperCol)/filterCol">
  ...
  <GlOrgUserSelect v-model:model-value="selectData"
