...
import {utils, fileTemplateApi, sysConfigApi, fileApi, isUtil} from '@geelato/gl-ui'
import type {PageQueryFilter, PageQueryRequest, QueryFileTemplateForm,} from '@geelato/gl-ui';
// 页面所需 对象、方法
import useLoading from '../../../hooks/loading';
import {type Pagination, PageSizeOptions} from '../../../types/global';
import {columns, enableStatusOptions, fileTypeOptions, useTypeOptions} from "./searchTable";
// 引入组件
...
const fetchData = async (params: PageQueryRequest) => {
  setLoading(true);
  try {
    const {data} = await fileTemplateApi.pageQueryFileTemplate(params);
...
const deleteData = async (id: string, successBack?: any, failBack?: any) => {
  try {
    await fileTemplateApi.deleteFileTemplate(id);
...
const generateFile = async (id: string, type: string) => {
  try {
    const {data} = await fileTemplateApi.generateTemplateOrMetaFile(id, type);
    reset();
  } catch (err) {
    console.log(err);
  }
...
const downloadImportOrExportExample = async (configKey: string) => {
  try {
    const {data} = await sysConfigApi.getValueByKeys(configKey);
    if (data) {
      if (isUtil.isJSON(data)) {
        fileApi.downloadFileByBase64Data(JSON.parse(data));
      } else {
        fileApi.fetchFileById(data, () => {
 ...
 * @param data
 */
const viewTable = (data: QueryFileTemplateForm) => {
  formParams.value = Object.assign(formParams.value, {
 ...
 * @param data
 */
const editTable = (data: QueryFileTemplateForm) => {
  formParams.value = Object.assign(formParams.value, {
 ...
 * @param data
 */
const deleteTable = (data: QueryFileTemplateForm) => {
 ...
 * @param type
 */
const saveSuccess = (data: QueryFileTemplateForm, type: string) => {
  if (type === 'add') {
    reset();
    emits('add', data);
  } else if (type === 'edit') {
        ...
        <a-row :gutter="wrapperCol">
          ...
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('security.file.index.form.fileCode')" field="fileCode">
              <a-input v-model="filterData.fileCode" allow-clear @clear="condition" @press-enter="condition"/>
            </a-form-item>
          </a-col>
