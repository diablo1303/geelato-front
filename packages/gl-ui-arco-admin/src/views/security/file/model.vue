...
import {fileTemplateApi, isUtil, applicationApi, fileApi} from '@geelato/gl-ui'
import type {QueryAppForm, QueryFileTemplateForm, AttachmentForm, Base64FileParams} from '@geelato/gl-ui';
import {enableStatusOptions, useTypeOptions,} from "./searchTable";
import UploadBase64 from "../../../components/upload-base64/index.vue";
import ImportBusinessType from "./import/type.vue";
import ImportBusinessRule from "./import/rule.vue";
import ImportBusinessMeta from "./import/meta.vue";
import ExportBusinessMeta from "./export/meta.vue";
...
/* 表单 */
const generateFormData = (): QueryFileTemplateForm => {
  return {
    id: props.modelValue || '',
    title: '',
    useType: '',
 ...
 * @param failBack
 */
const createOrUpdateData = async (params: QueryFileTemplateForm, successBack?: any, failBack?: any) => {
  ...
  if (!res) {
    try {
      ...
      const {data} = await fileTemplateApi.createOrUpdateFileTemplate(record);
...
const getData = async (id: string, successBack?: any, failBack?: any) => {
  try {
    const {data} = await fileTemplateApi.getFileTemplate(id);
    if (successBack && typeof successBack === 'function') successBack(data);
  } catch (err) {
    if (failBack && typeof failBack === 'function') failBack(err);
  }
...
const loadFiles = (attachmentIds: string, type: string) => {
  if (attachmentIds !== null && attachmentIds !== '' && !isUtil.isJSON(attachmentIds)) {
      ...
      if (attachs != null && attachs.length > 0) {
        attachs.forEach((value, index, array) => {
          if (value.delStatus === 0) {
            fileApi.fetchFileToBase64(value.id, (base64String: string) => {
              if (base64String) {
...
const saveOrUpdate = (successBack?: any, failBack?: any) => {
  createOrUpdateData(formData.value, (data: QueryFileTemplateForm) => {
    // 设计当前页面的操作
    if (successBack && typeof successBack === 'function') successBack(data);
  }, () => {
...
const loadPage = () => {
  ...
  if (['edit', 'view'].includes(props.formState) && props.modelValue) {
    getData(props.modelValue, (data: QueryFileTemplateForm) => {
      // 表格数据处理
...
<template>
  <a-tabs v-model:active-key="tabsKey" :default-active-tab="1" :lazy-load="true" :style="{height: `${height}px`}" position="left" type="line">
    ...
    <a-tab-pane v-if="['import'].includes(formData.useType)" :key="2" class="a-tabs-one" title="模板字段定义">
      <ImportBusinessType v-model="formData.businessTypeData as any[]" :disabled="formState==='view'" :height="height as number"/>
    </a-tab-pane>
    <a-tab-pane v-if="['export'].includes(formData.useType)" :key="2" class="a-tabs-one" title="数据配置">
      <ExportBusinessMeta v-model="formData.businessMetaData as any[]" :disabled="formState==='view'" :height="height as number"/>
    </a-tab-pane>
    <a-tab-pane v-if="['import'].includes(formData.useType)" :key="3" class="a-tabs-two" title="数据处理规则">
      <ImportBusinessRule v-model="formData.businessRuleData as any[]" :business-type-data="formData.businessTypeData as any[]"
                          :parameter="importParameter" :disabled="formState==='view'" :height="height as number"/>
    </a-tab-pane>
    <a-tab-pane v-if="['import'].includes(formData.useType)" :key="4" class="a-tabs-two" title="数据保存配置">
      <ImportBusinessMeta v-model="formData.businessMetaData as any[]" :business-type-data="formData.businessTypeData as any[]"
                          :parameter="importParameter" :disabled="formState==='view'" :height="height as number"/>
    </a-tab-pane>
  </a-tabs>
</template>
