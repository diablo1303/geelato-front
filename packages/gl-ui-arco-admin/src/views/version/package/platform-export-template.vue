...
import {utils, isUtil, fileApi} from '@geelato/gl-ui';
import type {PageQueryFilter, QueryFileTemplateForm} from '@geelato/gl-ui';
import {businessMetaDataValueComputeModeOptions, businessMetaDataValueTypeOptions} from "../../security/file/export/template";
import {businessMetaEvaluationOptions, businessRuleDataTypeOptions, businessTypeDataTypeOptions} from "../../security/file/import/template";
...
const parseJson = (record: string) => {
  return record && isUtil.isJSON(record) ? JSON.parse(record) : {};
}
      ...
      <a-table v-if="!selectedKeys || (selectedKeys.length > 0 && selectedKeys[0] === rootPid)"
        ...
        <template #columns>
          ...
          <a-table-column :ellipsis="true" :tooltip="true" :width="210" data-index="template" title="模板文件">
            <template #cell="{ record,item = parseJson(record.template) }">
              <a-button style="width: 18px;height: 18px;" type="text" @click="fileApi.downloadFileByBase64String(record.template)">
                <template #icon>
                  <IconDownload/>
                </template>
              </a-button>
          ...
          <a-table-column :ellipsis="true" :tooltip="true" :width="210" data-index="template_rule" title="模板文件源数据">
            <template #cell="{ record,item = parseJson(record.template_rule) }">
              <a-button style="width: 18px;height: 18px;" type="text" @click="fileApi.downloadFileByBase64String(record.template_rule)">
                <template #icon>
                  <IconDownload/>
                </template>
              </a-button>
              ...
              <template #columns>
                ...
                <a-table-column :ellipsis="true" :tooltip="true" :width="90" data-index="valueType" title="值类型">
                  <template #cell="{ record }">
                    {{ utils.getOptionLabel(record.valueType, businessMetaDataValueTypeOptions) }}
                  </template>
                </a-table-column>
                <a-table-column :ellipsis="true" :tooltip="true" :width="120" data-index="valueComputeMode" title="取值计算方式">
                  <template #cell="{ record }">
                    {{ utils.getOptionLabel(record.valueComputeMode, businessMetaDataValueComputeModeOptions) }}
                  </template>
                </a-table-column>
            ...
            <a-table :bordered="{cell:true}"
              ...
              <template #columns>
                ...
                <a-table-column :ellipsis="true" :tooltip="true" :width="180" data-index="type" title="规则类型">
                  <template #cell="{ record }">
                    {{ utils.getOptionLabel(record.type, businessRuleDataTypeOptions) }}
                  </template>
                </a-table-column>
            ...
            <a-table :bordered="{cell:true}"
              ...
              <template #columns>
                ...
                <a-table-column :ellipsis="true" :tooltip="true" :width="180" data-index="evaluation" title="字段值">
                  <template #cell="{ record }">
                    {{ utils.getOptionLabel(record.evaluation, businessMetaEvaluationOptions) }}
                  </template>
                </a-table-column>
