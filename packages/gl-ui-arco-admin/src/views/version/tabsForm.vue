...
import type {QueryAppForm, QueryAppVersionForm} from '@geelato/gl-ui';
...
import {type AppMeta, generateRenderData, type PageParams} from "../compare/type";
import CopyToClipboard from "../../components/copy-to-clipboard/index.vue";
import UploadFile from "../../components/upload-file/index.vue";
import PlatformApiList from "./package/platform-api.vue";
import PlatformDictList from "./package/platform-dict.vue";
import PlatformModelList from "./package/platform-model.vue";
import PlatformRoleList from "./package/platform-role.vue";
import PlatformSysConfigList from "./package/platform-sys-config.vue";
import PlatformExportTemplateList from "./package/platform-export-template.vue";
import PlatformEncodingList from "./package/platform-encoding.vue";
import PlatformResourcesList from "./package/platform-resources.vue";
import PlatformPageList from "./package/platform-page.vue";
import PlatformPacketList from "./package/platform-packet.vue";
...
const getData = async (id: string, successBack?: any, failBack?: any) => {
  try {
    const {data} = await versionApi.getAppVersion(id);
...
const getPackData = async (id: string, successBack?: any, failBack?: any) => {
  ...
  try {
    const {data} = await versionApi.queryAppPackage(id);
    if (successBack && typeof successBack === 'function') successBack(data);
  } catch (err) {
    if (failBack && typeof failBack === 'function') failBack(err);
  } finally {
...
<template>
  <a-card class="general-card" title="基本信息">
    ...
    <a-form :label-col-props="{ span: '2px' }" :model="tableData" class="form" style="padding-left: 16px;">
      ...
      <a-form-item field="packageSource" label="版本来源：">
        <span>{{ utils.getOptionLabel(tableData.packageSource, packageSourceOptions) }}</span>
      </a-form-item>
      <a-form-item field="status" label="版本状态：">
        <span>{{ utils.getOptionLabel(tableData.status, packageStatusOptions) }}</span>
      </a-form-item>
  ...
  <a-modal draggable  v-model:visible="tabsVisible" :footer="false" :title="`包详细信息 ${tableData.version}`" fullscreen title-align="start">
    <a-spin :loading="openTabsLoading" dot style="width: 100%" tip="正在获取版本包详细信息...">
      <a-tabs v-model:active-key="tabsKey" :default-active-tab="1" :lazy-load="true" position="top" type="line">
        ...
        <a-tab-pane :key="10" class="a-tabs-five" title="业务数据">
          <a-card v-if="listParams.visible" class="general-card6">
            <PlatformPacketList :height="listParams.height"
