...
import type {QueryAppVersionForm} from '@geelato/gl-ui';
import {versionApi} from '@geelato/gl-ui'
import {selectOptions} from "@geelato/gl-ui-arco";
import {packageSourceOptions} from "./searchTable";
...
/* 表单 */
const generateFormData = (): QueryAppVersionForm => {
  return {
    id: props.modelValue || '',
    version: '',
    packagePath: '',
 ...
 * @param failBack
 */
const saveData = async (params: QueryAppVersionForm, successBack?: any, failBack?: any) => {
  const res = await validateForm.value?.validate();
  if (!res) {
    try {
      const {data} = await versionApi.createOrUpdateAppVersion(params);
...
const validateVersion = async (value: any, callback: any) => {
  try {
    const {data} = await versionApi.validateAppVersion(formData.value);
...
const saveOrUpdate = (successBack?: any, failBack?: any) => {
  saveData(formData.value, (data: QueryAppVersionForm) => {
    // 设计当前页面的操作
    if (successBack && typeof successBack === 'function') successBack(data);
  }, () => {
...
const loadPage = () => {
  ...
  if (['edit', 'view'].includes(props.formState) && props.modelValue) {
    getData(props.modelValue, (data: QueryAppVersionForm) => {
      formData.value = data;
    });
  }
}
