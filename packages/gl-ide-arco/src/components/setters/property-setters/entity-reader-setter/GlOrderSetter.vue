    ...
    >
      <template #item="{element, index}">
        <tr>
          ...
          <td>
            <a-select size="small" v-model="mv[index].field" style="width: 8em"  @change="onChangeElement($event,element,index)">
              <a-option v-for="item in entityFieldMetas" :value="item.name" :title="item.name">{{item.title}}</a-option>
            </a-select>
          </td>
          <td>
            <a-select size="small" v-model="mv[index].order" style="width: 8em"  @change="onChangeElement($event,element,index)">
