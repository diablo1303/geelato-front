    ...
    >
      <template #item="{element, index}">
        <tr>
          ...
          <td>
            <a-select size="small" style="width: 10em" v-model="mv[index].name"  @change="onFieldChange($event,element,index)">
              <a-option v-for="item in entityFieldMetas" :value="item.name" :title="item.name+' '+item.title">{{item.name}}</a-option>
            </a-select>
          </td>
          <td>
            <a-input size="small" v-model="element.title" allowClear="true"
                     @change="onChangeElement($event,element,index)"
                     @click="onSelectElement($event,element,index)" style="width: 99%">
            </a-input>
          </td>
          <td style="width: 2em;text-align: center">
                  <span>
