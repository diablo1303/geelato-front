  ...
  <table class="gl-entity-reader-setter gl-table">
    ...
    <tr>
      <td class="gl-table-cell">
        <a-select v-model="mv.entity" @change="onEntityChange" allowSearch>
          <a-option v-for="item in entityStore.entityLiteMetas" :value="item.entityName"
            >{{ item.entityTitle }}
            {{ item.entityName }}
          </a-option>
        </a-select>
    ...
    <tr>
      <td class="gl-table-cell">
        <GlFieldsSetter
          v-model="mv.fields"
          :entityFieldMetas="entityStore.currentFieldMetas"
          :aliasOptions="options"
          @changeElement="onUpdate"
        ></GlFieldsSetter>
      </td>
    </tr>
    <!--=========================查询参数===========================-->
    ...
    <tr>
      <td
        ...
        title="数据加载的时机，一是组件初始化即加载，一是组件的加载数据方法被调用才加载"
      >
        加载时机？
      </td>
    </tr>
    <tr>
      <td class="gl-table-cell">
        <a-switch v-model="mv.immediate">
          <template #checked> 初化时</template>
          <template #unchecked> 调用时</template>
        </a-switch>
      </td>
    </tr>
    <tr>
      <td class="gl-table-cell gl-label" title="对于级联加载的数据可以配置懶加载即分层加载">
        懶加载？
      </td>
    </tr>
    <tr>
      <td class="gl-table-cell">
        <a-switch v-model="mv.lazyLoad">
          <template #checked> 分批次触发加载</template>
          <template #unchecked> 一次性加载完毕</template>
        </a-switch>
      </td>
    </tr>
    <tr>
