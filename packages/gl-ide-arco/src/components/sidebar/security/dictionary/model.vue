...
import {applicationApi, dictApi, useGlobal, utils} from "@geelato/gl-ui";
import type {QueryDictForm, QueryAppForm} from "@geelato/gl-ui";
...
const createOrUpdateData = async (params: QueryDictForm, successBack?: any, failBack?: any) => {
  ...
  if (!res) {
    try {
      const {data} = await dictApi.createOrUpdateDict(params);
...
const validateCode = async (value: any, callback: any) => {
  try {
    const {data} = await dictApi.validateDictCode(formData.value);
...
const loadPage = () => {
  ...
  if (['edit', 'view'].includes(props.formState) && props.modelValue) {
    getData(props.modelValue, (data: QueryDictForm) => {
      // 表格数据处理
...
<template>
  <a-form ref="validateForm" :label-col-props="{ span: labelCol }" :model="formData" :wrapper-col-props="{ span: wrapperCol }" class="form">
    <a-row :gutter="wrapperCol">
      ...
      <a-col :span="(labelCol+wrapperCol)/formCol">
        <a-form-item :rules="[{required: true,message: '这是必填项'}]" field="appId" label="应用">
      ...
      <a-col :span="(labelCol+wrapperCol)">
        <a-form-item :label-col-props="{ span: labelCol/formCol }" :wrapper-col-props="{ span: (labelCol+wrapperCol-labelCol/formCol) }" field="dictRemark"
                     label="备注">
