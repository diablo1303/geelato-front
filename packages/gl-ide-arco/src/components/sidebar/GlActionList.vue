...
const openActionSetter = (inst: ComponentInstance, action: Action, actionIndex?: number) => {
  ...
  const foundActionMeta = componentMeta.actions.find((actionMeta: ActionMeta) => {
    return actionMeta.name == action.name
  })
  ...
  // 等100ms,再弹出动作页面
  utils.sleep(200).then(() => {
    emitter.emit(EventNames.GlIdeOpenActionEditor, {
      action,
      actionIndex,
  ...
  <div class="gl-action-list">
    ...
    <a-collapse :default-active-key="['1']" :bordered="false">
      ...
      >
        ...
        <div v-for="(action, actionIndex) in inst.actions">
          <div class="gl-action-item">
            <div class="gl-title" @click="openActionSetter(inst, action, actionIndex)">
          ...
          <div v-for="beRefAction in beRefActions[action.id]" :key="beRefAction.action.id">
            <a-tooltip
              :content="`以上事件（${action.title}），被此事件引用，该事件来源于组件（${beRefAction.label}）`"
              position="right"
            >
              <div class="gl-action-item">
                <GlIconfont type="gl-link" style="margin: 0 0.5em"></GlIconfont>
                <div class="gl-title" @click="openRefActionSetter(beRefAction)">
                  {{ beRefAction.action.title }}：{{ beRefAction.action.name }}
                </div>
                <div style="flex: 0 0 2em; text-align: center; line-height: 2em">
                  <GlIconfont
                    type="gl-thunderbolt"
                    class="gl-active"
                    style="cursor: pointer"
                    @click="openRefActionSetter(beRefAction)"
                  />
                </div>
              </div>
            </a-tooltip>
