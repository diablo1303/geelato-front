import type {PageQueryRequest, PageQueryResponse, QueryResult} from "./Base";
...
export function pageQueryConnects(params: Record<string, any>) {
  const records = utils.getUrlParams(params);
  return entityApi.getAxios().get<PageQueryResponse>(`/api/model/connect/pageQuery?${records.join('&')}`);
}
...
export function getConnect(id: string) {
  return entityApi.getAxios().get<QueryConnectForm>(`/api/model/connect/get/${id}`);
}

export function createOrUpdateConnect(params: QueryConnectForm) {
  return entityApi.getAxios().post<QueryResult>('/api/model/connect/createOrUpdate', params);
}

export function jdbcConnect(params: QueryConnectForm) {
  return entityApi.getAxios().post<QueryResult>('/api/model/connect/jdbcConnect', params);
}

export function deleteConnect(id: string) {
  return entityApi.getAxios().delete<QueryResult>(`/api/model/connect/isDelete/${id}`);
}
...
export function pageQueryTables(params: Record<string, any>) {
  const records = utils.getUrlParams(params);
...
export function queryDefaultView(entityName: string) {
  return entityApi.getAxios().get<string>(`/api/model/table/queryDefaultView/${entityName}`);
}
...
export function pageQueryTableForeigns(params: Record<string, any>) {
  const records = utils.getUrlParams(params);
  return entityApi.getAxios().get<PageQueryResponse>(`/api/model/table/foreign/pageQuery?${records.join('&')}`);
}

export function queryTableForeigns(params: Record<string, any>) {
  const records = utils.getUrlParams(params);
  return entityApi.getAxios().get<QueryTableForeignForm[]>(`/api/model/table/foreign/query?${records.join('&')}`);
}
...
export function pageQueryViews(params: Record<string, any>) {
  const records = utils.getUrlParams(params);
  return entityApi.getAxios().get<PageQueryResponse>(`/api/model/view/pageQuery?${records.join('&')}`);
}
...
export function queryAppViews(params: Record<string, any>) {
  const records = utils.getUrlParams(params);
  return entityApi.getAxios().get<QueryAppViewForm[]>(`/api/app/view/query?${records.join('&')}`);
}

export function getAppView(id: string) {
  return entityApi.getAxios().get<QueryAppViewForm>(`/api/app/view/get/${id}`);
}

export function createOrUpdateAppView(params: QueryAppViewForm) {
  return entityApi.getAxios().post<QueryResult>('/api/app/view/createOrUpdate', params);
}

export function deleteAppView(id: string) {
  return entityApi.getAxios().delete<QueryResult>(`/api/app/view/isDelete/${id}`);
}
...
export const getCommonFieldsOptions = async (successBack?: any, failBack?: any) => {
  try {
    const {data} = await queryTableColumns({tableName: 'platform_common_fields', enableStatus: 1} as unknown as PageQueryRequest);
    if (successBack && typeof successBack === 'function') successBack(data);
  } catch (err) {
    if (failBack && typeof failBack === 'function') failBack(err);
  }
