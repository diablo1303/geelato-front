...
import {schemaArco, schemaBlock} from "@geelato/gl-ui-schema-arco";
import {DndProvider} from 'vue3-dnd'
import ComponentBuilderExample from "./ComponentBuilderExample.vue";

const componentId = ref('')
componentId.value = new URL(window.location.href).searchParams.get("componentId") || "";

const componentMetas = ref(schemaArco.componentMetas)
const schema = ref('schemaArco')
const change = () => {
  componentMetas.value = schema.value === 'schemaArco' ? schemaArco.componentMetas : schemaBlock.componentMetas
  if (!searchText.value) {
  ...
  renderComponentMetas.value = componentMetas.value.filter((meta) => {
    return (meta.title.indexOf(searchTextStr) !== -1) || (meta.componentName.toLowerCase().indexOf(searchTextStr) !== -1)
  })
}
  ...
  <div class="ide-components">
    ...
    <table class="gl-table" style="width: 100%">
      <tr>
        ...
        <th class="gl-table-cell gl-label" style="min-width: 6em">是否有属性</th>
        <th class="gl-table-cell gl-label" style="min-width: 8em">操作</th>
      </tr>
      <tr v-for="(componentMeta,index) in renderComponentMetas" :key="index" style="margin-bottom: 4px">
        <td class="gl-table-cell">{{ index + 1 }}</td>
        <td class="gl-table-cell"><span style="width: 30em">{{ componentMeta.title }} {{
