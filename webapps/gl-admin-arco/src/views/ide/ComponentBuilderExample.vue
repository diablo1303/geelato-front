<template>
  ...
  <div>
    <a-modal
        v-if="componentMetaRegisterVisible" v-model:visible="componentMetaRegisterVisible" :destroyOnClose="true"
        title="注册组件"
        class="gl-component-register-modal" fullscreen>
      <GlComponentBuilder :componentMeta="componentMeta" :componentInstance="componentInstance"
                          :action="componentMetaAction"></GlComponentBuilder>
      <template #footer>
...
export default {
  ...
  data() {
    return {
      ...
      // ls: localStorage,
      // 当前选中的组件元数据
      currentComponentMeta: this.componentMeta,
      currentComponentInstance: this.componentInstance,
      // 组件元数据操作，是新增，还是修改
