...
// 页面所需 对象、方法
import {
  deleteTableForeign as deleteList,
  pageQueryTableForeigns as pageQueryList,
  QueryTableForeignForm as QueryForm
} from '@/api/model';
import {columns, enableStatusOptions} from './searchTable';
// 引入组件
import ModelTableForeignForm from './form.vue';
...
const props = defineProps({
  ...
  height: {type: Number, default: 0}, // 列表 - 数据列表高度，滑动条高度
});
...
watch(() => props, (val) => {
  if (props.visible === true) {
    ...
    formParams.value.parameter = {
      connectId: props.parameter?.connectId || '', mainTable: props.parameter?.tableName || '',
...
<template>
  <ModelTableForeignForm v-model:visible="formParams.visible"
        ...
        <a-row :gutter="wrapperCol">
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('model.foreign.index.form.mainTableCol')" field="mainTableCol">
              <a-input v-model="filterData.mainTableCol" allow-clear @clear="condition($event)" @press-enter="condition($event)"/>
            </a-form-item>
          </a-col>
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('model.foreign.index.form.foreignTable')" field="foreignTable">
              <a-input v-model="filterData.foreignTable" allow-clear @clear="condition($event)" @press-enter="condition($event)"/>
            </a-form-item>
          </a-col>
          <a-col :span="(labelCol+wrapperCol)/filterCol">
            <a-form-item :label="$t('model.foreign.index.form.foreignTableCol')" field="foreignTableCol">
              <a-input v-model="filterData.foreignTableCol" allow-clear @clear="condition($event)" @press-enter="condition($event)"/>
            </a-form-item>
          </a-col>
          <a-col :span="(labelCol+wrapperCol)/filterCol">
  ...
  <a-table
    ...
    <template #columns>
      <a-table-column :title="$t('model.foreign.index.form.index')" :width="70" align="center" data-index="index">
