...
import {getRoleSelectOptions, pageQueryRoleUserOf, QueryRoleForm} from "@/api/security";
...
const queryRoleByUser = async (params: Record<string, any>, successBack?: any, failBack?: any) => {
  try {
    const {data} = await pageQueryRoleUserOf({
      ...params,
      ...
      <a-row :gutter="wrapperCol">
        <a-col v-if="!!formData.orgId" :span="(labelCol+wrapperCol)/formCol">
          <a-form-item label="所属组织" field="orgId">
            <OrgChooseBox v-model="formData.orgId" :disabled="true"/>
            <template #help>
              当前用户所属的组织，限定选择人员的范围，
            </template>
          </a-form-item>
        ...
        <a-col :span="(labelCol+wrapperCol)/formCol">
          <a-form-item label="选择人员" field="userId">
            <UserChooseBox v-model="formData.userId" :max-count="1" :parameter="props.parameter" :root-org-id="formData.orgId"/>
          </a-form-item>
        </a-col>
        <a-col :span="(labelCol+wrapperCol)/formCol">
