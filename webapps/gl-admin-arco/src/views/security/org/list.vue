<template v-model="pageData">
  ...
  <a-table row-key="id" column-resizable
           :loading="loading"
    ...
    <template #columns>
      ...
      <a-table-column :title="$t('sercurity.org.index.form.operations')" data-index="operations" align="center" fixed="right"
                      :width="pageData.formState==='edit'?230:100">
        <template #cell="{ record }">
          <a-button v-permission="['admin']" type="text" size="small" @click="viewTable(record.id)">
            {{ $t('searchTable.columns.operations.view') }}
          </a-button>
          <a-button v-show="pageData.formState==='edit'" v-permission="['admin']" type="text" size="small" @click="editTable(record.id)">
            {{ $t('searchTable.columns.operations.edit') }}
          </a-button>
          <a-popconfirm :content="$t('searchTable.columns.operations.deleteMsg')" position="tr" type="warning" @ok="deleteTable(record.id)">
            <a-button v-show="pageData.formState==='edit'" v-permission="['admin']" type="text" size="small">
...
import {deleteOrg as deleteList, FilterOrgForm as FilterForm, PageQueryFilter, pageQueryOrg as pageQueryList, PageQueryRequest} from '@/api/sercurity_service'
// 引用其他页面
...
/* 对外调用方法 */
const loadList = (action: string, pageSize?: number, parentId?: string, closeBack?: any) => {
  pageData.value.formState = action || 'edit';
  basePagination.pageSize = pageSize || pageData.value.pageSize;
  reset();
}
// 将方法暴露出去
