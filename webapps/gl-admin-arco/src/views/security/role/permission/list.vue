<template v-model="pageData">
        ...
        <a-row :gutter="16">
          <a-col v-show="false" :span="24">
            <a-form-item v-show="false">
              <a-input v-show="false" v-model="filterData.permissionId"/>
          ...
          <a-col :span="pageData.isModal?12:8">
            <a-form-item :label="$t('security.rolePermission.index.form.permissionName')" field="permissionName">
              <a-input v-model="filterData.permissionName" allow-clear @clear="search($event)" @press-enter="search($event)"/>
            </a-form-item>
          </a-col>
          <a-col :span="pageData.isModal?12:8">
  ...
  <a-table :bordered="{cell:true}" :columns="(cloneColumns as TableColumnData[])"
    ...
    <template #columns>
      <a-table-column :title="$t('security.rolePermission.index.form.index')" :width="80" align="center" data-index="index">
        <template #cell="{  rowIndex }">{{ rowIndex + 1 + (pagination.current - 1) * pagination.pageSize }}</template>
      </a-table-column>
      <a-table-column :ellipsis="true" :title="$t('security.rolePermission.index.form.permissionName')" :tooltip="true" :width="150"
                      data-index="permissionName"/>
      <a-table-column v-if="pageData.params.roleName===''" :ellipsis="true" :title="$t('security.rolePermission.index.form.roleName')" :tooltip="true"
                      :width="150" data-index="roleName"/>
      <a-table-column :title="$t('security.rolePermission.index.form.createAt')" :width="180" data-index="createAt"/>
  ...
  <RolePermissionForm ref="rolePermissionFormRef"></RolePermissionForm>
</template>
...
import {deleteRolePermission as deleteList, FilterRolePermissionForm as FilterForm, pageQueryRolePermission as pageQueryList} from '@/api/security';
...
const generateFilterData = (): FilterForm => {
  return {id: '', roleId: '', roleName: '', permissionId: '', permissionName: '', createAt: []};
};
