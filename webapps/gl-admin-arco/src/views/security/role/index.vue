<template>
  <div class="container">
    ...
    <Breadcrumb :items="['sercurity.role.index.menu.list', 'sercurity.role.index.menu.list.searchTable']"/>
    <!-- 卡片 Card 将信息分类后分标题、详情等区域聚合展现，一般作为简洁介绍或者信息的大盘和入口。   -->
    <a-card :title="$t('sercurity.role.index.menu.list.searchTable')" class="general-card">
      <!-- list   -->
...
const loadIndex = () => {
  if (roleListRef.value) {
    roleListRef.value?.loadList({action: 'edit'});
  }
}
