...
import {copyToClipboard, generateRandom} from "@/utils/strings";
import {
  batchCreateOrUpdateDictItem,
  pageQueryDictItem as pageQueryList,
  QueryDictItemForm,
  QueryDictItemForm as QueryForm
} from "@/api/security";
import {enableStatusOptions} from "@/views/security/dictionary/item/searchTable";
...
const validateForm = async (): Promise<boolean> => {
  ...
  if (columnData.value && columnData.value.length > 0) {
    const {data} = await pageQueryList({
  ...
  try {
    if (columnData.value && columnData.value.length > 0) {
      ...
      await validateForm().then(async (valid: boolean) => {
        if (valid) {
          await batchCreateOrUpdateDictItem(props.parameter.dictId, props.parameter.pid, columnData.value);
