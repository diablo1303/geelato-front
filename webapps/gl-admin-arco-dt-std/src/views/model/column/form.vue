...
// 引入组件
import ModelTableColumnModel from './model.vue';
...
const tableFormRef = shallowRef(ModelTableColumnModel);
...
<template>
  <a-modal v-if="isModal"
           ...
           :title="title || $t(`model.column.index.model.title.${formState}`)"
           :width="width || ''" title-align="start"
           @cancel="handleModelCancel($event)" @before-ok="handleModelOk">
    <ModelTableColumnModel ref="tableFormRef"
